...
for i in range(30):
    ...
    gt_sent=test_sp_texts[i].replace("[start]","").replace("[end]","").lower()
    translated = decode_sequence(input_sentence).replace("[start]","").replace("[end]","").lower()
    print("Input:%s\tTranslation:%s\tGround truth:%s"%(input_sentence, translated, gt_sent))
    spanish_pred = [w for w in translated.split(" ") if w!=""]
